...
router.get("/htmx/create-new-template", htmxOnlyEndpoint, requiresAuthentication, (request, responds) => {
  return ( responds
    ...
    .send(/*HTML*/`
      ...
      <br>
      <form hx-put="/api/v1/template" hx-trigger="submit" class="relative flex flex-col" id="template-creation" hx-target="#content" hx-swap="afterbegin">
        <div>
          <div class="form-group flex-grow">
            <!-- ID -->
          ...
          <div class="flex justify-center mt-6">
            <button class="default confirm" type="button" id="new-template-preview-button">
              ...
              <script class="hidden">
                ...
                document.getElementById("new-template-preview-button").addEventListener("click", updateFrameURL);
              </script>
            </button> 
          </div>
          <div class="flex justify-center mt-6">
      ...
      <br>
      <iframe id="new-template-iframe" src="/api/v1/template/preview?name=preview&json=[]" frameborder="0" class="w-full h-[100vh]"></iframe>
