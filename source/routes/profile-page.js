  ...
  return (/*HTML*/`
    <div class="absolute bottom-4 top-4 left-4 right-4 border xl:border-none border-gray-300 rounded-lg p-3 xl:p-0">
      <div class="xl:grid xl:grid-cols-2 xl:gap-4 xl:h-full">
        <section class="xl:h-full xl:w-full xl:border xl:rounded-md xl:border-gray-300 xl:p-3" id="profile-explanation">
...
export function profileFormComponent(request) {
  return (/*HTML*/`
    <form id="user-${request?.session?.passport?.user}-profile-form"  class="relative flex flex-col" hx-trigger="submit" hx-post="/api/v1/user/by-id/${request?.session?.passport?.user}" hx-target="#update-results">            
      <div class="flex-1">
      ...
      <div class="p-3 grid grid-cols-2 w-full mt-auto">
        <div class="flex justify-center">
          <button hx-get="/profile/htmx/form" hx-trigger="click" hx-swap="outerHTML" hx-target="#user-${request?.session?.passport?.user}-profile-form" class="default cancel">
