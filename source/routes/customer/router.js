    ...
    .send(/*HTML*/`
      <section id="actions">
        <form class="inline-menu" action="get">
          <input class="inline-menu" type="radio" id="action1" name="action" hx-trigger="click" hx-get="${URLs.base}" hx-target="#main" hx-push-url="true" ${ originalUrlWithoutQuery === URLs.base ? "checked" : "" }>
          <label class="inline-menu" for="action1">Simple Customer Search</label>
          <input class="inline-menu" type="radio" id="action2" name="action" hx-trigger="click" hx-get="${URLs.search}" hx-target="#main" hx-push-url="true" ${ originalUrlWithoutQuery === URLs.search ? "checked" : "" }>
          <label class="inline-menu" for="action2">Advanced Customer Search</label>
          <input class="inline-menu" type="radio" id="action3" name="action" hx-trigger="click" hx-get="${URLs.create}" hx-target="#main" hx-push-url="true" ${ originalUrlWithoutQuery === URLs.create ? "checked" : "" }>
