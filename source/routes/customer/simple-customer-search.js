export default function simpleCustomerSearch(request) {
  return (/*HTML*/`
    <form hx-get="/api/v1/customer/all" hx-target="#customers-list" hx-trigger="submit" class="w-full flex" id="customer-search-form">
      <label for="customer-name" class="inline-block" ><p class="flex justify-center">Search:</p></label>
      <input  id="customer-name" class="w-full mx-3 border-gray-300 rounded-lg border p-2" type="text" name="name" placeholder="Search for a customer by name...">
      <button type="submit">
