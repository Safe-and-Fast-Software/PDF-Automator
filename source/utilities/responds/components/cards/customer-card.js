...
export default function customerCardComponent(customer) {
  ...
  return (/*HTML*/`
    <li id="${cardID}" class="list-none w-full border border-gray-300 rounded-lg">
      <form hx-post="${apiEndpoint}" hx-trigger="submit" class="relative flex flex-col p-4" hx-on::after-request="${clearTextAfter3Seconds}" hx-target="#${customerUpdateRespondsID}">
        <div>
        ...
        <div class="p-3 grid grid-cols-3 w-full mt-auto">
          <div class="flex justify-center">
            <button hx-delete="${apiEndpoint}" hx-target="#${cardID}" hx-confirm="Are you sure you want to delete '${customer?.name}'? The consequences of this action cannot be undone." hx-trigger="click" hx-swap="delete" class="default delete">
